...
class PayPalPaymentsForm(forms.Form):
    ...
    # Default fields.
    cmd = forms.ChoiceField(widget=forms.HiddenInput(), initial=CMD_CHOICES[1][0])
    ...
    def __init__(self, items, button_type="buy", *args, **kwargs):
        super(PayPalPaymentsForm, self).__init__(*args, **kwargs)
        self.button_type = button_type
        custom_val = ""
        for i, item in enumerate(items):
            self.fields['amount_'+str(i+1)] = forms.CharField(widget=forms.HiddenInput(), initial=str(item.price))
            self.fields['item_name_'+str(i+1)] = forms.CharField(widget=forms.HiddenInput(), initial=str(item.name))
...
</form>""" % (POSTBACK_ENDPOINT, self.as_p(), settings.MEDIA_URL+"img/cart/paypal.png"))
    ...
    %s
    <input type="image" src="%s" border="0" name="submit" alt="Buy it Now" />
</form>""" % (SANDBOX_POSTBACK_ENDPOINT, self.as_p(), settings.MEDIA_URL+"img/cart/paypal.png"))
